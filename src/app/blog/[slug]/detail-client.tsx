...
export function BlogDetailPageClient({ post, fillRelated }: Props) {
  const { language, t } = useLanguage();

  const title = language === "vi" ? post.title : post.titleEn;
  const excerpt = language === "vi" ? post.excerpt : post.excerptEn;
  const author = language === "vi" ? post.author : post.authorEn;

  const articleContentVi = (
  ...
  const articleContentEn = (
    <div className="prose-tamloi mt-8 space-y-5 text-[15px] leading-relaxed text-[var(--color-ink)]">
      <p>
        In modern life, frozen foods are increasingly becoming a smart choice
        that helps save time while still ensuring nutrition for the whole family.
        However, not everyone knows how to choose safe and high-quality frozen foods.
      </p>

      <h2 className="font-serif text-2xl font-semibold text-[var(--color-ink)]">
        1. Why choose frozen foods correctly?
      </h2>
      <p>
        Fast freezing at low temperatures helps retain maximum flavor, nutrition,
        and freshness. When produced and preserved correctly, frozen foods are
        absolutely safe — sometimes even retaining more vitamins and minerals
        than fresh food left for a long time.
      </p>

      <h2 className="font-serif text-2xl font-semibold text-[var(--color-ink)]">
        2. Criteria for choosing safe frozen foods
      </h2>
      <ul className="list-inside list-disc space-y-2">
        <li>
          <strong>Clear origin:</strong> prioritize reputable brands certified
          internationally under HACCP and ISO 22000:2018.
        </li>
        <li>
          <strong>Intact packaging:</strong> no tears, no signs of defrosting and refreezing.
        </li>
        <li>
          <strong>Complete labeling:</strong> clear manufacturing date, expiry date, storage and cooking instructions.
        </li>
        <li>
          <strong>Stable storage:</strong> supermarket freezers must maintain -18°C or below.
        </li>
      </ul>

      <h2 className="font-serif text-2xl font-semibold text-[var(--color-ink)]">
        3. Closed production process — the key to quality
      </h2>
      <p>
        At Tam Loi Foods, every product undergoes a 5-step closed production process:
        source ingredients, quality inspection, pre-processing, fast freezing, and packaging.
        The entire process is strictly monitored according to international standards,
        ensuring food safety and preserving traditional Vietnamese flavor.
      </p>
    </div>
  );
    ...
    <>
      <nav className="bg-white py-3 text-[12px]" aria-label="Breadcrumb">
        <ol className="container-page flex flex-wrap items-center gap-1.5 text-[var(--color-ink-muted)]">
          ...
          <li className="text-[var(--color-ink-soft)]">›</li>
          <li className="line-clamp-1 text-[var(--color-ink)]">{title}</li>
        </ol>
      </nav>
      ...
      <section className="bg-[var(--color-bg-soft)] py-10 md:py-12">
        <div className="container-page grid gap-8 md:grid-cols-12">
          <article className="md:col-span-8">
            ...
            <Reveal>
              {language === "vi" ? articleContentVi : articleContentEn}
            </Reveal>
            ...
            <Reveal delay={150}>
              <blockquote className="mt-10 rounded-2xl border-l-4 border-[var(--color-brand)] bg-[var(--color-cream)] p-6 md:p-8">
                ...
                <p className="mt-3 font-serif text-lg italic leading-relaxed text-[var(--color-ink)] md:text-xl">
                  {language === "vi" 
                    ? "Mỗi bữa ăn ngon lành là một lời cảm ơn dành cho gia đình. Chọn thực phẩm đúng cách là cách chăm sóc người thân yêu một cách trọn vẹn nhất."
                    : "Every delicious meal is a message of thanks to the family. Choosing food correctly is the most complete way to care for your loved ones."}
                </p>
